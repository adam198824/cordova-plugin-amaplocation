...
static NSString* const ADDRESSLIST_KEY     = @"addressList";
static NSString* const PROVINCE_KEY    = @"province";
static NSString* const ROAD_KEY        = @"road";
/**
    ...
    if(manager == curLocationManager){
        ...
        [geocoder reverseGeocodeLocation:location completionHandler:^(NSArray<CLPlacemark *> *_Nullable placemarks, NSError * _Nullable error) {
            if(manager){
                ...
                [locationInfoDic setObject:[NSString stringWithFormat:@"%f",gcjLocation.latitude] forKey:LATITUDE_KEY];
                [locationInfoDic setObject:[NSString stringWithFormat:@"%f",gcjLocation.longitude] forKey:LONGITUDE_KEY];
                
                [locationInfoDic setObject:address forKey:ADDRESS_KEY];
                [locationInfoDic setObject:[self stringNullDealWith:placemark.administrativeArea] forKey:PROVINCE_KEY];
                [locationInfoDic setObject:[self stringNullDealWith:placemark.thoroughfare] forKey:ROAD_KEY];
                
                NSDictionary *addressListDic = @{
                                                 @"province":[self stringNullDealWith:placemark.administrativeArea],
                                                 @"city":[self stringNullDealWith:placemark.locality],
                                                 @"district":[self stringNullDealWith:placemark.subLocality],
                                                 @"street":[self stringNullDealWith:placemark.name],
                                                 };
                [locationInfoDic setObject:addressListDic forKey:ADDRESSLIST_KEY];
                if(callbackId != nil){
                    CDVPluginResult* pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:locationInfoDic];
...
///判断是否是null 如果是空返回@""
-(NSString *)stringNullDealWith:(id)str
{
    ...
    NSString *returnStr = [NSString stringWithFormat:@"%@",str];
    return returnStr;
}
